...
<div class="copyright" style="margin-top:20px; display: none;"><a>Privacy</a> - <a>Terms</a> - <a>About</a> - <a
  ...
  copyright is Debox Global | All rights reserved</div>
...
<div id="successmessage-outer" style="display: none;" class="warningmessage">
  <div id="successmessage" style="background-color: #C02621!important;">
    <div class="msgimg">
      <i class="fa fa-check" aria-hidden="true"></i>
    </div>
    <div class="successmsg-heading">
    ...
    <div class="donebtn" onclick="closeserrormsg();" style="background-color: #C02621!important;">
      OK
    </div>
  </div>
</div>
...
">New Delhi Institute of Management</p>
  <ul style="margin: -6px 0px 7px 0px !important;">
    <li><a href="<?php echo $fullurl; ?>timeline.html"><i class="fa fa-home" aria-hidden="true"></i></a>Home</li>
...
// Run query directly with mysqli (assuming $conn = getDbConnection())
$resOptions = mysqli_query($conn, $sqlOptions);

if ($resOptions && mysqli_num_rows($resOptions) > 0) {
  while ($rowOptions = mysqli_fetch_assoc($resOptions)) {
    $slideText .= '"' . $rowOptions["membershipText"] . '",';
  }
}

// Agar last comma hataani ho to:
$slideText = rtrim($slideText, ',');
?>
<script>
...
<?php
$fpage = isset($_GET['fpage']) ? $_GET['fpage'] : 0;


if (isset($_SESSION["sessUserId"]) && $_SESSION["sessUserId"] != '' && $_SESSION["sessUserId"] != 0 && $fpage != 1) {
  ...
  if ($pageIndex != 3) {
    ...
    // MySQLi query
    $sql_contact = "SELECT * FROM " . _CONTACT_MASTER_TABLE_ . " WHERE userId = $userId AND status = 1";
        ...
        <div style="border-top:2px #00a0af solid;">
          <div><span id="chatfieldbox"><textarea name="chatfieldfooter" id="chatfieldfooter" type="text"
            ...
            <form class="edit-layer" enctype="multipart/form-data" name="frmposthome2" id="frmposthome2" method="post"
              ...
              <input type="hidden" name="contactchatuserid" id="contactchatuserid" value="<?php echo isset($userId2) ? $userId2 : $_SESSION['sessUserId']; ?>" />
            ...
            <ul class="emozi">
              ...
              <li class="send-sec"><span id="sendpressenter">Press Enter to Send</span> <button class="send"
                  id="sendbuttonchat" onclick="clicktosendchat();" style="background-color: #C02621;">Send</button>
        ...
        <div id="showinvitediv">
          ...
          <div class="nw-grup">
            ...
            <input type="hidden" name="groupinvitationid" id="groupinvitationid"
              value="<?php echo encodeStr(isset($_REQUEST['groupId']) && $_REQUEST['groupId']); ?>">
          ...
          <div>
            <div style="font-weight:normal;">
              <div style="float:left;">&copy; <?php echo date("Y"); ?>&nbsp;NDIM VECOSPACE &nbsp;|
                &nbsp;Powered by &nbsp;&nbsp;&nbsp;</div>
              <div><a href="https://corrintech.com/"><img src="images/Logo De Boxpng.png" style="width:50px;"></a></div>
            </div>
          </div>
...
<script>
  function opencallreplay() {
    ...
    window.open(
      '<?php echo $fullurl; ?>konectt-live.html?r=' + vcallroomname +
      '&u=' + vcalluserId +
      '&ac=1&t=<?php echo $TokenRequest; ?>',
      '<?php echo addslashes($companNameTitle); ?> Video Call',
      'directories=no,titlebar=no,toolbar=no,location=no,status=no,menubar=no,scrollbars=no,resizable=no,width=960,height=600'
    );
  }
...
<script>
  ...
  var runtimweee = setInterval(function () {
    //$('#cmntmsgtimeleftdiv').load('timeload.php'); //Then set it to run again after ten minutes
  }, 600000);
...
<div id="checkscmntmsgtimeleftdiv" style="display:none;"></div>
<div id="cmntmsgtimeleftdiv" style="display:none;"></div>
...
<script>
  function showmoreless(id) {
    ...
    if (text == 'read more') {
      $('#shortdesc' + id).css('height', 'auto');
      $('#readtext' + id).text('read less');
      $('#hideafterreadmore' + id).remove();
    }
    else {
      $('#shortdesc' + id).css('height', '140px');
