...
<h3>Interests</h3><a class="add_btn"
	onClick="$('#editinterest').show();$('#defaultinterest').hide();addnewinterest(<?php echo $countinterest; ?>,100);$('#interestcontentboxs').hide();"><i
		class="fa fa-plus-circle" aria-hidden="true"></i>
	<?php if ($countinterest > 1) { ?>Edit<?php } else { ?>Add<?php } ?></a>
...
<?php if ($countinterest == 1) { ?>
	<div class="add_more" id="defaultinterest">
		...
		<span class="addmore_icon"
			onClick="$('#editinterest').show();$('#defaultinterest').hide();addnewinterest(<?php echo $countinterest; ?>,100);$('#interestcontentboxs').hide();">
...
<div class="edit_box" id="editinterest" style="display:none;">
	<form class="edit-layer" method="post" target="actionfrm" action="<?php echo $fullurl; ?>common_action.php">
		<h2></h2>
		<a onClick="$('#editinterest').hide();$('#defaultinterest').show();$('#interestcontentboxs').show();"
		...
		<ul>
			...
			if ($resLogin) {
				while ($row = mysqli_fetch_array($resLogin)) {
					...
					<li id="newinterestli<?php echo $countinterest; ?>"><input type="text"
							id="newinterest<?php echo $countinterest; ?>" name="check_list[]"
							value="<?php echo sanitizedboutput($row["interestText"]); ?>"
							placeholder="Add new interest and experience"
							onKeyUp="addnewinterest(Number(<?php echo $countinterest; ?>+1),100);">
