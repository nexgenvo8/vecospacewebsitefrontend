...
<div class="promotion-cont">
	...
	<form class="edit-layer" enctype="multipart/form-data" name="frmpostad" id="frmpostad" method="post"
		...
		<div class="promote-left">
			...
			<div class="form-group" id="adcategorydiv">
				...
				<select name="adcategory" id="adcategory" onChange="selectadcategory(this.value);choosemyad();">
					<option value="0" <?php if ($postcat == 0) {
						echo "selected";
					} ?>>Select</option>
					<!--<option value="6" <?php if ($postcat == 6 || $postcat == 'p') {
						echo "selected";
					} ?>>I want to promote my Post</option>-->
					<option value="7" <?php if ($postcat == 7 || $postcat == 'a') {
						echo "selected";
					} ?>>I want to promote my
						Article</option>
					<option value="1" <?php if ($postcat == 1) {
						echo "selected";
					} ?>>I want to promote my Event</option>
					<option value="2" <?php if ($postcat == 2) {
						echo "selected";
					} ?>>I want to promote my PROjects</option>
					<option value="3" <?php if ($postcat == 3) {
						echo "selected";
					} ?>>I want to promote my SMB</option>
					<option value="4" <?php if ($postcat == 4) {
						echo "selected";
					} ?>>I want to promote my Talent</option>
					<option value="5" <?php if ($postcat == 5) {
						echo "selected";
					} ?>>I want to promote my Jobs</option>
				</select>
			</div>
			...
			<div class="form-group" id="adbtnlinkdiv" style="display:none;">
				...
				<input type="text" name="addlinkurl" id="addlinkurl" onKeyUp="choosemyad();" maxlength="250"
					autocomplete="off" placeholder="Enter the URL you want to promote">
			</div>
			<script>
			...
			<div class="form-group budget" id="mybudgetdiv" style="display:none;">
				...
				if ($resEvents) {
					while ($rowEvents = mysqli_fetch_array($resEvents)) {
						...
						?>
						<label><input type="radio" name="budgetId" id="budgetId"
								value="<?php echo stripslashes($rowEvents["id"]); ?>" <?php if ($g == 1) {
									  echo 'checked';
								  } ?>><?php echo stripslashes($rowEvents["duration"]); ?> - INR
							<?php echo stripslashes($rowEvents["amount"]); ?></label>
						<?php
						$g++;
					}
				}
