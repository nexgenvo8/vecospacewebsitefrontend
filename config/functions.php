<?php
session_start();
$conn = getDbConnection(); // connection open karna
...
if (isset($_SESSION['sessUserId']) && !empty($_SESSION['sessUserId'])) {
	...
	$timezoneget = mysqli_fetch_assoc($hb);
...
function mobileclean($str)
{
	return str_replace("&", "&amp;", trim(strip_tags(stripslashes($str))));
}
...
function insertDB($tableName, $insertArray, $valArray, $isQuery = "", $sqlQuery = "")
{
	...
	if (!empty($isQuery) && is_string($isQuery) && strtolower($isQuery) === "yes") {
		$strQuery = $sqlQuery;
	} else {
		if (!is_array($insertArray)) {
			echo "Fields array incorrect";
			exit();
		}

		if (!is_array($valArray)) {
			echo "Val array incorrect";
			exit();
		}

		if (count($insertArray) !== count($valArray)) {
			notifyqueryerror(
				__LINE__,
				implode(",", $insertArray) . "=====" . implode(",", $valArray),
				mysqli_error($conn),
				mysqli_errno($conn),
				__FILE__,
				"insert fields"
			);
			echo "Fields and Val Fields do not match";
			exit();
		}

		for ($i = 0; $i < count($insertArray); $i++) {
			$strInsert .= ($strInsert === "") ? $insertArray[$i] : "," . $insertArray[$i];
		}
		...
		for ($i = 0; $i < count($valArray); $i++) {
			...
			if (is_string($val)) {
				...
				$escapedVal = $val; // keep numeric values untouched
			}

			$strVal .= ($strVal === "") ? "'$escapedVal'" : ",'$escapedVal'";
		}
	...
	if (trim($strQuery) !== "") {
		$res = mysqli_query($conn, $strQuery);

		if ($res) {
			return mysqli_insert_id($conn); // return last inserted ID
		} else {
			notifyqueryerror(__LINE__, $strQuery, mysqli_error($conn), mysqli_errno($conn), __FILE__, "insert");
			return -1;
		}
	} else {
...
function updateDB($tableName, $insertArray, $valArray, $whereArray, $wherevalArray, $isQuery, $sqlQuery)
{
	...
	if ($isQuery == "yes") {
		...
		if (countArray($insertArray) != countArray($valArray)) {
			...
			notifyqueryerror(__LINE__, implode(",", $insertArray) . "=====" . implode(",", $valArray), mysqli_error(), mysqli_errno(), __FILE__, "update fields");
			echo $errorMsg;
			exit();
		}
	...
	if (trim($strQuery) != "") {
		...
		$res = mysqli_query($conn, $strQuery) or notifyqueryerror(
			__LINE__,
			$strQuery,
			mysqli_error($conn),
			mysqli_errno($conn),
...
function deleteDB($tableName, $whereArray, $wherevalArray, $isQuery, $sqlQuery)
{
	...
	if (trim($strQuery) != "") {
		...
		$res = mysql_query(query: $strQuery) or notifyqueryerror(__LINE__, $strQuery, mysqli_error(), mysqli_errno(), __FILE__, "delete");
		if ($res) {
			return true;
		} else {
			return false;
...
function getRecords($tableName, $fieldArray, $whereArray, $valsArray, $isQuery, $sqlQuery, $orderBy = '', $orderSort = '', $limit = '', $newOffset = '')
{
	...
	if (trim($strQuery) != "") {
		...
		$res = mysqli_query($conn, $strQuery) or error_found(mysqli_errno($conn));
...
function getTableRecordCount($tableName)
{
	...
	// Escape table name for safety (better to validate against whitelist)
	$tableName = mysqli_real_escape_string($conn, $tableName);

	$strSql = "SELECT COUNT(*) AS cnt FROM " . $tableName;

	$resRecords = mysqli_query($conn, $strSql);

	if ($resRecords) {
		$row = mysqli_fetch_assoc($resRecords);
		$rowCount = $row['cnt'] ?? 0;
		mysqli_free_result($resRecords);
	} else {
		die("Error fetching record count: " . mysqli_error($conn));
	}
...
{
	global $conn; // make $conn available here
	$str = trim($str);
	$str = stripslashes($str);
	return mysqli_real_escape_string($conn, $str);
}

function sanitizetoinsert($conn, $str)
{
	$str = trim($str);
...
function clean($str, $conn = null)
{
	if ($str === null) {
		$str = ''; // prevent null warning
	}
	$str = trim((string) $str); // cast to string safely
	if ($conn) {
		return mysqli_real_escape_string($conn, $str);
	}
	return $str;
...
function wordclean($conn, $input)
{
	...
	$output = str_replace(array_keys($replace_pairs), array_values($replace_pairs), $input);

	return mysqli_real_escape_string($conn, $output);
}
...
function getusername($id, $colName)
{

	$reAct = "select $colName from " . _USERS_MASTER_TABLE_ . " WHERE userId='" . $id . "'";
...
function image_fix_orientation($filename)
{
	...
	$mime = $info['mime'];
	if ($mime != 'image/jpeg') {
		return; // Only JPEG has EXIF data
	}
...
function generate_image_thumbnail($sourceFile, $destFile, $thumbWidth, $thumbHeight)
{
	...
	switch ($mime) {
		case 'image/jpeg':
			imagejpeg($thumbImage, $destFile, 90);
			break;
		case 'image/png':
			imagepng($thumbImage, $destFile);
			break;
		case 'image/gif':
			imagegif($thumbImage, $destFile);
			break;
	}
...
function makedatetime($ptime)
{
	if ($ptime == 0 || $ptime == '') {
		...
		// ensure $ptime is numeric timestamp
		if (!is_numeric($ptime)) {
...
{
	$siteurl = 'https://www.connecwrk.com/';
...
{
	global $conn; // Make sure MySQLi connection is available
	...
	$total_pages_data = mysqli_fetch_assoc($result_count);
	$total_pages = isset($total_pages_data['num']) ? $total_pages_data['num'] : 0;
	...
	// RETURN results
	return [$result, $total_pages, $paginate];
} // ← Closing brace added
...
function adminnotification($subject, $mailBodyContent)
{


	$email = "konecttnotifications@gmail.com";
...
function chattimelineentry($userId, $contactId, $chatType)
{
	/*$dateAdded=time();
	$total=0;
	$sqlCheck="";
	$sqlCheck="select id from "._CHAT_TIMELINE_TABLE_." where userId='".$userId."' and contactId='".$contactId."' and chatType='".$chatType."'";
	$resQuery=mysql_query($sqlCheck) or die(mysql_error());
	$totaResQuery=mysql_num_rows($resQuery);
	if($totaResQuery<1)
	{

	$sql_ins="insert into "._CHAT_TIMELINE_TABLE_." set userId='".$userId."',contactId='".$contactId."',dateAdded=".$dateAdded.",chatType='".$chatType."'";
	mysql_query($sql_ins) or die(mysql_error());
	}
	else
	{
	$sql_ins="update "._CHAT_TIMELINE_TABLE_." set dateAdded=".$dateAdded." where userId='".$userId."' and contactId='".$contactId."' and chatType='".$chatType."'";
	mysql_query($sql_ins) or die(mysql_error());
	}*/

}

function profileborder($usersType)
{
	if ($usersType == 1) {
		$border = '3px solid #f9eb3f';
	} else if ($usersType == 2) {
		$border = '3px solid #a5f192';
	} else if ($usersType == 3) {
		$border = '3px solid #baf1f7';
	} else if ($usersType == 4) {
		$border = '3px solid #ffbfce';
	} else if ($usersType == 5) {
		$border = '3px solid #bfeb0f';
	} else {
		$border = '';
	}
	return $border;
