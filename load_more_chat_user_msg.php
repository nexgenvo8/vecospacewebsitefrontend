...
if ($startfrom > 0) {
	...
	<div id="pageid<?php echo $pageid + 1; ?>"><a class="loadmoremessagesclass"
			onclick="loadmorechatbox('<?php echo ($startfrom - 20); ?>','<?php echo $userId2; ?>');">Load more messages</a>
	</div>
...
$sqlLogin = "";
$offset = max(0, $startfrom - 20);

$sqlLogin = "SELECT * FROM " . _CHAT_MASTER_TABLE_ . "
             WHERE userId='" . $_SESSION['sessUserId'] . "'
             AND contactId='" . decodeStr($userId2) . "'
