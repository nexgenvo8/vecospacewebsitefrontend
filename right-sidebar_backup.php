			...
			<div class="birth-namecont">
				...
				?>
						...
						<div class="birthcont">
							...
							<div class="birth-nm">
								<h2>It's <?php echo stripslashes(trim($rowcontacts["firstName"])) . "&rsquo;s"; ?> Birthday Today!</h2>
								<span><?php echo stripslashes($rowcontacts["jobTitle"]); ?> <?php if (stripslashes($rowcontacts["companyName"]) != '') {
				...
				<a <?php if ($mb > 1) { ?>onclick="funcommonpopupwin('520px','auto','<?php echo $fullurl; ?>common_popup_inner.php?type=sayhappybirthday','Say Happy Birthday');" <?php } else { ?> onclick="openuserchatbox('<?php echo $usercontactid; ?>','<?php echo $userfirstName; ?> <?php echo $userlastName; ?>','<?php echo $fullurl; ?>profile/<?php echo $usercontactid; ?>/<?php echo $frienddobnameurl; ?>.html');$('#chatfieldfooter').val('Happy Birthday');$('#shb').val('1');" <?php } ?> class="send-wish">Send Message</a>
			</div>
		</div>
					...
					<ul class="trending-list">
						...
						?>
								<li style="cursor:pointer;" onclick="window.location.href = '<?php echo $fullurl; ?>view-article.html?postId=<?php echo encodeStr($rowViewArticle2['id']); ?>';">
									...
									<a class="ttle"><?php echo substr((stripslashes(trim($rowViewArticle2["postTitle"]))), 0, 5000); ?></a>
									<span class="desc"><?php echo substr(strip_tags(stripslashes(trim($rowViewArticle2["postText"]))), 0, 6000); ?></span>
