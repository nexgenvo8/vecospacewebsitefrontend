...
if (trim($_REQUEST['commentbox']) != '' && $_REQUEST['parentId'] != '' && $_REQUEST['postId'] != '' && $_REQUEST['action'] == 'postcomment') {
	...
	if ($onlineStatus != 1) {
		...
		$resCmnsts = mysqli_query(getDbConnection(), $sqlCmnsts);
		...
		if ($getCmnsts["dateAdded"] < strtotime("-10 minutes", time()))//You can send an email after 10 minutes according last message
		{
			...
			$getSql = mysqli_query(getDbConnection(), $sql) or die(error_found(mysqli_error(getDbConnection())));
...
if ($_SESSION["sessUserId"] != '' && $_SESSION["sessUserId"] != 0) {
	...
	?>
	<?php if ($totalpostcomment > 5 && $_REQUEST['limit'] == 5) { ?>
	...
	$a = "";
	$a = "select * from " . _COMMENT_MASTER_TABLE_ . " where postId=" . $_REQUEST['postId'] . "  and commentText!='' and postType=" . $_REQUEST['postType'] . " and parentId=0 order by id desc limit " . $startlmit . "," . $limit . "";
	...
	while ($comment = mysqli_fetch_array($b)) {
		...
		$b2 = mysqli_query(getDbConnection(), $a2) or die(mysqli_error(getDbConnection()));
		...
		$b235 = mysqli_query(getDbConnection(), $a235) or die(mysqli_error(getDbConnection()));
		...
		<li id="cmntid<?php echo $comment['id']; ?>">
			...
			$ab = "";
			$ab = "select * from " . _COMMENT_MASTER_TABLE_ . " where postId=" . $_REQUEST['postId'] . " and commentText!='' and postType=" . $_REQUEST['postType'] . " and parentId=" . $comment['id'] . " ";
