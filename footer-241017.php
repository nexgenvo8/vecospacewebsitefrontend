...
<div class="copyright" style="margin-top:20px; display: none;"><a>Privacy</a> - <a>Terms</a> - <a>About</a> - <a href="<?php echo $fullurl;?>faq.html">FAQ's</a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;  &copy; <?php echo date("Y");?>  The copyright is OMSR Media Pvt. Ltd | All rights reserved</div>
...
<div class="foottr">
  <ul>
    ...
    <li><a href="<?php echo $fullurl;?>myprofile/<?php echo encodeStr($_SESSION['sessUserId']);?>/<?php echo $userurl;?>.html"><img src="<?php echo $fullurl;?>uploads/<?php echo $myprofilePhoto; ?>"></a></li>
  </ul>
</div>
...
<div onclick="$('#selectbuttons').hide();" class="chat-cont" <?php if($contactrequests<1){ ?>style="display:none;"<?php } ?>>
  <div class="chat-cont-header" id="mainfriedboxlist" onclick="hideshowfooterchatbox();"><span id="msgheaderfooter">+</span>Messaging <span class="minimiz"> <a class="bulkmsg" onclick="funcommonpopupwin('520px','auto','<?php echo $fullurl; ?>common_popup_inner.php?type=sendbulkemails','New message');"><i class="fa fa-pencil-square-o" aria-hidden="true"></i></a>
   ...
   <div class="chatsearchmsg" style="display:none;"><i class="fa fa-search" aria-hidden="true"></i><input type="text" name="chatsearch" id="chatsearch" onkeyup="searchcontactfun();" placeholder="Search" /></div>
...
<div style="width:336px; display:none; position:fixed; right:380px; bottom:0px; z-index:999; background-color:#FFFFFF; box-shadow: 0px 0px 5px #a0a0a0;" id="usermainboxchat">
<div class="chat-cont-header"  >
<span id="userchatname" onclick="$('#loaduserchatbox').toggle();"></span>
<a href="#" id="videocallinguserIcon" onclick="$('#videocallinguserIcon').hide();" target="_blank" ><span class="vdo-icon">&nbsp;</span></a>
<span class="minimiz" onclick="$('#loaduserchatbox').toggle();"><i class="fa fa-minus" aria-hidden="true"></i></span>
<span class="close-cht" style="position:absolute;font-size:inherit; right:14px; top:8px;"onclick="$('#usermainboxchat').hide();clearint();$('#shb').val('0');"><i class="fa fa-times" aria-hidden="true"></i></span> 
<div class="vdocall-msg" style="display:none;">You are in this video call<div class="chat-btns">
 <button onclick="Declinecall();$('.chat-btns').hide();$('.chat-btns').hide();$('.vdocall-msg').hide();$('.vdo-icon').removeClass('active');">Decline</button>
 <a href="<?php echo $fullurl; ?>konectt-live.html?ac=1" target="_blank" onclick="$('.chat-btns').hide();stopring();"><button>Answer</button></a>
</div></div>
</div>
<div  id="loaduserchatbox">
<div style="height:256px; overflow:auto;" id="loadchatusermsg"></div>
<div style="border-top:2px #1a94c3 solid;">
<div><textarea name="chatfieldfooter" id="chatfieldfooter" type="text" style="border:0px; max-width:100%;width: 100%;max-height: 100px; padding:10px; border:0px;" placeholder="Write a message here..."  onclick="$('#selectbuttons').hide();" maxlength="800" /></textarea>

 <form class="edit-layer" enctype="multipart/form-data" name="frmposthome2" id="frmposthome2" onSubmit="$('#sharepopup').show();showloading('sharepopinner');" method="post" target="actionfrm" action="<?php echo $fullurl;?>common_action.php"> 
	   <input name="myphotofile" id="myphotofile" type="file" onChange="$('#frmposthome2').submit();" style="display:none;" accept="image/x-png,image/gif,image/jpeg,application/msword, application/vnd.ms-excel, application/vnd.ms-powerpoint,text/plain, application/pdf">
	    <input type="hidden" name="action" id="action" value="uploadgroupimagesssssssss" />
		<input type="hidden" name="groupId" id="groupIdssssss" value="<?php echo $_REQUEST["groupId"];?>" />
...
<ul class="emozi">
         ...
         <li class="emogi">
          ...
          <li class="atach"><a id="sendphoto"><i class="fa fa-paperclip" aria-hidden="true"></i></a></li>
         <li class="send-sec"><span id="sendpressenter">Press Enter to Send</span> <button class="send" id="sendbuttonchat" onclick="clicktosendchat();">Send</button>
         <div class="click-btn"><a id="sendclick"><i class="fa fa-ellipsis-h" aria-hidden="true" onclick="$('#selectbuttons').toggle();" id="openpressenter"></i></a>
...
<div class="share-popup" id="sharepopup">
  <div class="sare-img-pop">
    ...
    <div class="popup-fttr" style="padding: 20px;"> <a onclick="$('.share-popup').hide();$('#posturl').val('');removefilefunc();">Cancel</a>
            ...
            <div class="crt-grp-popup" style="display: none;" id="groupformdiv">
              <div class="popup-inner">
               ...
               <a class="grp-close" onClick="$('.crt-grp-popup').hide();$('body').css('overflow','auto');">
                <img src="<?php echo $fullurl;?>images/Close.svg"></a>
                <form enctype="multipart/form-data" name="creategroup" id="creategroup" method="post" target="actionfrm" action="<?php echo $fullurl;?>common_action.php" onsubmit="formValidation('creategroup');return false"/>
                <div class="nw-grup" >
                ...
                <input type="text" name="groupName" id="groupName" class="validate" placeholder="e.g. 'Marketing specialists Warsaw'" maxlength="100">
               <script>
			   $("#groupName").focus();
			   </script>
			    <div id="textareadiv">
				<label>Short group description</label>
                <textarea name="groupDetails" id="groupDetails" rows="4" style="height:100px; margin-bottom:5px;" placeholder="Describe your group " maxlength="150"></textarea>
<div style="margin-bottom:10px;">Max 150 Characters</div></div>
				<div><input type="checkbox" checked="checked" disabled="disabled" /> Yes, I agree to and accept the code of <a href="<?php echo $fullurl;?>conduct-guidelines.html" target="_blank">conduct for moderators</a>.</div>
       		    </div>
                <div class="popup-fttr"> <a onClick="$('.crt-grp-popup').hide();$('#groupName').val('');$('#groupDetails').val('');$('body').css('overflow','auto');">Cancel</a>
			...
			<div class="crt-grp-popup" id="inviteemail">
              ...
              <div class="invit">
               ...
               <form enctype="multipart/form-data" name="inviteemailaddress" id="inviteemailaddress" method="post" target="actionfrm" action="<?php echo $fullurl;?>common_action.php" />
                <div class="nw-grup">
                <h3>Invite by email</h3>		
				<label>Email address<span class="reqstar">*</span></label>
                <input type="text" name="txtuseremail" id="txtuseremail" placeholder="Enter email address with comma separated" maxlength="250" class="validate">

				<input type="hidden" name="action" id="action" value="sendtouserinvitation">
				<input type="hidden" name="groupinvitationid" id="groupinvitationid" value="<?php echo encodeStr($_REQUEST['groupId']);?>">
				<input type="hidden" name="invitegroupname" id="invitegroupname" value="<?php echo stripslashes(trim($rowGroup["groupName"]));?>">
       		    </div>
                <div class="popup-fttr"> <a onclick="$('#inviteemail').hide();$('#txtuseremail').val('');$('#sentmsgtextdiv').hide();">Cancel</a>
                  <button type="button" onClick="formValidation('inviteemailaddress');">Send invitation</button>
                </div>
                </form>
				</div>
                ...
                <div class="popup-fttr"> <a onclick="$('#inviteemail').hide();$('#txtuseremail').val('');$('#sentmsgtextdiv').hide();">Close</a>
                  <button type="submit" onclick="$('#hideinvitediv').hide();$('#showinvitediv').show();">More invitation</button>
                </div>
                </div>
				</div>
                </div>
...
$(document).mouseup(function(e) { var container = $(".remove-list"); 
 if (!container.is(e.target) && container.has(e.target).length === 0) { container.hide(); } });
</script>

<div class="crt-grp-popup" id="grouppostpopup" style="display: none;">
    <div class="popup-inner">
    <a class="grp-close" onClick="$('.crt-grp-popup').hide();$('#groupPostTitle').val('');$('#groupPostText').val('');"><i class="fa fa-times" aria-hidden="true"></i></a>
	
    <form class="nw-grup" enctype="multipart/form-data" name="frmposthome" id="frmposthome" method="post" target="actionfrm" action="<?php echo $fullurl;?>common_action.php"> 
      <h3>Write a group post</h3>
      <input type="text" name="groupPostTitle" id="groupPostTitle" placeholder="Write a title for group post" maxlength="100" class="validate">
      <textarea rows="4" name="groupPostText" id="groupPostText" placeholder="Write group post" style="margin-bottom:5px;"></textarea>
     <div class="post-img">
	...
	<form class="nw-grup" enctype="multipart/form-data" name="imagegroupfrmposthome" id="imagegroupfrmposthome" method="post" target="actionfrm" action="<?php echo $fullurl;?>common_action.php" style="display:none;">
	<input name="groupimagefilehome" id="groupimagefilehome"  type="file"  onChange="$('#commonloader').show();$('#imagegroupfrmposthome').submit();"  accept="image/x-png,image/gif,image/jpeg" style="display:none;">
