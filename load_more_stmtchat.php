...
{
?>
<div id="pageid<?php echo $pageid+1;?>"><a class="loadmoremessagesclass"  onclick="loadmorestmtchat('<?php echo ($startfrom-20);?>','<?php echo $userId2;?>');">Load more messages</a></div>
...
$sqlTotal = mysqli_num_rows(
    mysqli_query(
        $conn,
        "SELECT id FROM " . _STUDENT_MENTOR_CHAT_MASTER_TABLE_ . " 
...
$sqlLogin="";
$sqlLogin="select * from "._STUDENT_MENTOR_CHAT_MASTER_TABLE_." where userId='".$_SESSION['sessUserId']."' and contactId='".decodeStr($userId2)."' ORDER BY dateAdded ASC LIMIT ".($startfrom-20).",".$startfrom."";
...
<?php if($img==0){  ?><div class="chatmsg<?php if (strpos($row["chatText"], 'maps/place') !== false) { ?> iframehave<?php } ?>"><?php echo nl2br(showsmily($row["chatText"]));?></div><?php } else { ?><div class="imgbox"><img src="<?php echo $fullurl;?>uploads/<?php echo $row["chatFileName"]; ?>" style="width:200px;"  onClick="imagepopupmain('<?php echo 'x_'.$row["chatFileName"];?>');" /></div><?php } ?>
