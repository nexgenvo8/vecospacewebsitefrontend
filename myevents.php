...
if ($resEvents && mysqli_num_rows($resEvents) > 0) {
    while ($rowEvents = mysqli_fetch_assoc($resEvents)) {
        ...
        <li id="<?php echo trim($rowEvents['id']); ?>">
            <a href="<?php echo $fullurl; ?>events-detail.html?eventId=<?php echo encodeStr($rowEvents['id']); ?>" class="block">
                <div class="evnt_box">
                    <div class="evnt-img">
                    ...
                    <div class="evnt-dtail">
                        ...
                        <div class="dropdwn">
                            <a href="javascript:void(0);" class="open" onclick="$('.remove-list').hide();$('#openc<?php echo trim($rowEvents['id']); ?>').show();">
                            ...
                            <ul class="remove-list" id="openc<?php echo trim($rowEvents['id']); ?>" style="display: none;">
                                <li><a href="<?php echo $fullurl; ?>post-events.html?eventId=<?php echo encodeStr($rowEvents['id']); ?>"><i class="fa fa-pencil" aria-hidden="true"></i>Edit</a></li>
                                <li><a style="cursor:pointer;" onclick="copyToClipboard('#clipboard<?php echo ($rowEvents['id']); ?>');"><i class="fa fa-link" aria-hidden="true"></i> Copy link to post</a></li>
                                <li><a onclick="alertpopupmain('<?php echo encodeStr($rowEvents['id']); ?>','delevnt');"><i class="fa fa-trash-o" aria-hidden="true"></i> Remove Event</a></li>
                            </ul>
                        </div>
                    </div>
                </div>
...
<div style="
    ...
    float: left;
    line-height: 22px;
">Thank you for your interest in posting your event on our platform. We are reviewing the details submitted by you. Upon approval, we would send you an email and your event will be uploaded within 24 hours. We wish you a successful event.<br>
