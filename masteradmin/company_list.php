...
$sql_ins="update "._COMPANY_MASTER_TABLE_." set status='$status',emailSent=1 where id = ".$id."";			
  ...
  <div style="margin:auto; width:600px; background-color:#FFFFFF; text-align:left;">
    <div style="padding:30px;">
      ...
      <div style="padding:10px; background-color:#F9F9F9; border:dashed 1px #ccc; border-radius: 2px;">
        ...
        <div style="display: block;width: 100%;margin-top: 15px;">Congratulations! The profile for your company "'.$_REQUEST['pagetitle'].'" is now visible on Konectt.<br>
          <br>
         You can access it at any time by going to '.$_REQUEST['pageurl'].' or through the menu bar on the left side and by clicking on "Corporate Connect".<br><br>
 Your Company name will be visible on the right side under the heading "Your Company Pages".
</div>
          </div>
        </div>
...
$subject='Your Company Profile has been activated'; 
...
$classlist=1;			
	 $query = "SELECT COUNT(*) as num FROM "._COMPANY_MASTER_TABLE_."  where companyName!='' order by id desc ";			
	...
	$query1 = "SELECT * FROM "._COMPANY_MASTER_TABLE_." where companyName!='' order by id desc LIMIT $start, $limit";			
	...
	<div class="listingbox" style="position:relative; min-height:300px;">			
	  <table width="100%" border="0" cellpadding="0" cellspacing="0">			
        <tr>
          ...
          <td width="33%" align="left" valign="top" class="listingheaddr">Company Name</td>
		...
		<tr <?php if($classlist==1){ ?>style="background-color:#f7f7f7;" <?php }  else { ?>style="background-color:#FFFFFF;" <?php } ?>>
          ...
          <td align="left" valign="middle" class="graylist"><?php  echo stripslashes($res_post['companyName']); ?></td>
...
?></td>
          ...
          <td align="center" valign="middle" class="graylist"><?php if($res_post['status']=='1'){ ?>			
            <a href="<?php echo $backpage; ?>?status=0&id=<?php echo base64_encode($res_post['id']); ?>&unino=<?php echo $_SESSION["cust_session_token"]."_console";?>&emailSent=<?php echo $res_post["emailSent"];?>&email=<?php  echo stripslashes($user_res['email']); ?>&username=<?php echo $user_res['firstName'].' '.$user_res['lastName']; ?>" onclick="globalloading();"><img src="images/lock.png" /></a>			
            <?php } else { ?><a href="<?php echo $backpage; ?>?status=1&id=<?php echo base64_encode($res_post['id']); ?>&unino=<?php echo $_SESSION["cust_session_token"]."_console";?>&pageurl=<?php echo $websiteurl;?>corporate-connect/<?php echo encodeStr($res_post['id'])."/".$postTitle.".html";?>&pagetitle=<?php  echo stripslashes($res_post['companyName']); ?>" onclick="globalloading();"><img src="images/unlock.png" width="30" border="0" /></a><?php } ?></td>			
          <td align="center" valign="top" class="graylist"><a href="<?php echo $websiteurl;?>corporate-connect/<?php echo encodeStr($res_post['id'])."/".$postTitle.".html";?>" target="_blank"><img src="images/eye_inv.png" alt="View" width="30" height="32" border="0" /></a></td>
        </tr>			
