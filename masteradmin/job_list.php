...
$sql_ins="update "._JOBS_MASTER_TABLE_." set status='$status',emailSent=1 where id = ".$id."";			
  ...
  <div style="margin:auto; width:600px; background-color:#FFFFFF; text-align:left;">
    <div style="padding:30px;">
      ...
      <div style="padding:10px; background-color:#F9F9F9; border:dashed 1px #ccc; border-radius: 2px;">
        ...
        <div style="display: block;width: 100%;margin-top: 15px;">Congratulations! The Job listing that you created is now visible on Konectt. You can edit the particulars at any time and delete the listing as well.<br>
          <br>
         You can access the same through the left side menu bar, by clicking on "Corporate Connect". Your Company name will be visible on the right side under the heading "Your Company Pages". Click on your company name and then on the "Jobs" tab. If you have multiple company pages, please click on the company for which you have created the job listing for. <br>
          <br>You may also search the Job listing directly under the "Jobs" section on Konectt.
</div>
          </div>
        </div>
...
$subject='Your Job posted successfully on Konectt'; 
...
$classlist=1;			
	 $query = "SELECT COUNT(*) as num FROM "._JOBS_MASTER_TABLE_."  where jobStatus=1 order by id desc ";			
	...
	$query1 = "SELECT * FROM "._JOBS_MASTER_TABLE_." where jobStatus=1  order by id desc LIMIT $start, $limit";			
	...
	<div class="listingbox" style="position:relative; min-height:300px;">			
	  <table width="100%" border="0" cellpadding="0" cellspacing="0">			
        <tr>
          ...
          <td align="left" valign="top" class="listingheaddr"> Title </td>
          <td align="left" valign="top" class="listingheaddr">Location</td>
		...
		<tr <?php if($classlist==1){ ?>style="background-color:#f7f7f7;" <?php }  else { ?>style="background-color:#FFFFFF;" <?php } ?>>
          ...
          <td align="left" valign="middle" class="graylist"><?php  echo stripslashes($res_post['jobLocation']); ?></td>
...
?></td>
          <td align="center" valign="middle" class="graylist"><?php if($res_post['status']=='0'){ ?>			
            <a href="<?php echo $backpage; ?>?status=1&id=<?php echo base64_encode($res_post['id']); ?>&unino=<?php echo $_SESSION["cust_session_token"]."_console";?>&emailSent=<?php echo $res_post["emailSent"];?>&email=<?php  echo stripslashes($user_res['email']); ?>&username=<?php echo $user_res['firstName'].' '.$user_res['lastName']; ?>" onclick="globalloading();"><img src="images/lock.png" /></a>			
            <?php } else { ?><a href="<?php echo $backpage; ?>?status=0&id=<?php echo base64_encode($res_post['id']); ?>&unino=<?php echo $_SESSION["cust_session_token"]."_console";?>&emailSent=<?php echo $res_post["emailSent"];?>&pageurl=<?php echo $websiteurl;?>view-job.html?id=<?php echo encodeStr($res_post['id']);?>" onclick="globalloading();"><img src="images/unlock.png" width="30" border="0" /></a><?php } ?></td>			
          <td align="center" valign="top" class="graylist"><a href="<?php echo $websiteurl;?>view-job.html?id=<?php echo encodeStr($res_post['id']);?>" target="_blank"><img src="images/eye_inv.png" alt="View" width="30" height="32" border="0" /></a></td>
        </tr>			
