...
$sql_ins="update "._TIMELINE_MASTER_TABLE_." set status='$status' where postId = ".$id."";			
...
<style type="text/css">
.postadminimg img,.postadminimg iframe {
    display: none;
}
</style>		
...
$classlist=1;			
	 $query = "SELECT COUNT(*) as num FROM "._SHAREANDUPDATES_TABLE_."  where postType='2' order by id desc ";			
	...
	$query1 = "SELECT * FROM "._SHAREANDUPDATES_TABLE_." where postType='2' order by dateAdded desc LIMIT $start, $limit";			
	...
	<div class="listingbox" style="position:relative; min-height:300px;">			
	  <table width="100%" border="0" cellpadding="0" cellspacing="0">			
        <tr>
          ...
          <td width="33%" align="left" valign="top" class="listingheaddr">Post Title </td>
          ...
          <td width="17%" align="center" valign="top" class="listingheaddr">Blocked</td>
          ...
          <td width="5%" align="center" valign="top" class="listingheaddr">Post </td>
        </tr>			
        <?php  while($res_post = mysql_fetch_array($result)) { 
		$postTitle=substr(cleanInput(strip_tags(trim(strtolower(str_replace(' ','-',preg_replace('/[^A-Za-z0-9\-]/', '-', $res_post['eventName'])))))),0,100);
		?>			
		<tr <?php if($classlist==1){ ?>style="background-color:#f7f7f7;" <?php }  else { ?>style="background-color:#FFFFFF;" <?php } ?>>
          ...
          <td align="left" valign="middle" class="graylist postadminimg"><?php  echo strip_tags(stripslashes($res_post['postText'])); ?></td>
          <td align="left" valign="middle" class="graylist"><?php $re="select * from "._USERS_MASTER_TABLE_."  where userId='".$res_post['userId']."'";
...
?></td>
          ...
          <td align="center" valign="middle" class="graylist"><?php $query2="select COUNT(*) as num from "._HIDDEN_POSTS_MASTER_TABLE_."  where timeLineId='".$res_post['id']."'";
$total_block = mysql_fetch_array(mysql_query($query2));			
			
	echo $total_block = $total_block[num];
?></td>
          <td align="center" valign="middle" class="graylist"><?php  echo date('d/m/Y H:i:s',$res_post['dateAdded']); ?></td>
          <td align="center" valign="middle" class="graylist"><?php $re3="select * from "._TIMELINE_MASTER_TABLE_."  where postId='".$res_post['id']."'";
$re3=mysql_query($re3) or die(mysql_error());  $user_res3=mysql_fetch_array($re3);?>
<?php if($user_res3['status']=='1'){ ?>			
            <a href="<?php echo $backpage; ?>?status=0&id=<?php echo base64_encode($res_post['id']); ?>&unino=<?php echo $_SESSION["cust_session_token"]."_console";?>" onclick="globalloading();"><img src="images/unlock.png" width="30" border="0" /></a>			
            <?php } else { ?><a href="<?php echo $backpage; ?>?status=1&id=<?php echo base64_encode($res_post['id']); ?>&unino=<?php echo $_SESSION["cust_session_token"]."_console";?>" onclick="globalloading();"><img src="images/lock.png" /></a><?php } ?></td>			
          <td align="center" valign="top" class="graylist"><a href="<?php echo $websiteurl;?>single-post.html?postId=<?php echo encodeStr($res_post['id']);?>&postType=<?php echo $res_post['postType'];?>" target="_blank"><img src="images/eye_inv.png" alt="View" width="30" height="32" border="0" /></a></td>
          
		  <td align="center" valign="middle" class="graylist"><?php $re3="select * from "._TIMELINE_MASTER_TABLE_."  where postId='".$res_post['id']."'";
$re3=mysql_query($re3) or die(mysql_error());  $user_res3=mysql_fetch_array($re3);?>
<?php if($user_res3['status']=='1'){ ?>			
            <a href="<?php echo $backpage; ?>?status=0&id=<?php echo base64_encode($res_post['id']); ?>&unino=<?php echo $_SESSION["cust_session_token"]."_console";?>" onclick="globalloading();" style="background-color: #ff0000; color: #fff; padding: 5px; font-weight: 100; border-radius: 3px;">Discard</a>			
            <?php } else { ?><a href="<?php echo $backpage; ?>?status=1&id=<?php echo base64_encode($res_post['id']); ?>&unino=<?php echo $_SESSION["cust_session_token"]."_console";?>" onclick="globalloading();" style="background-color: #41ad49; color: #fff; padding: 5px; font-weight: 100; border-radius: 3px;">Approve</a><?php } ?></td>
