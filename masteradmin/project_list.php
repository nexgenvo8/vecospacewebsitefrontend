	...
	<div class="listingbox" style="position:relative; min-height:300px;">			
	  <table width="100%" border="0" cellpadding="0" cellspacing="0">			
		...
		<tr <?php if($classlist==1){ ?>style="background-color:#f7f7f7;" <?php }  else { ?>style="background-color:#FFFFFF;" <?php } ?>>
          ...
          <td align="left" valign="middle" class="graylist"><?php  echo stripslashes($res_post['projectTitle']); ?></td>
...
?></td>
          ...
          <td align="center" valign="middle" class="graylist"><?php if($res_post['finalPost']=='1'){ ?>			
            <a href="<?php echo $backpage; ?>?status=0&id=<?php echo base64_encode($res_post['id']); ?>&unino=<?php echo $_SESSION["cust_session_token"]."_console";?>&pageurl=<?php echo $websiteurl;?>preview-project.html?projId=<?php echo encodeStr($res_post['id']);?>" onclick="globalloading();"><img src="images/unlock.png" width="30" border="0" /></a>			
            <?php } else { ?><a href="<?php echo $backpage; ?>?status=1&id=<?php echo base64_encode($res_post['id']); ?>&unino=<?php echo $_SESSION["cust_session_token"]."_console";?>&emailSent=<?php echo $res_post["emailSent"];?>&email=<?php  echo stripslashes($user_res['email']); ?>&username=<?php echo $user_res['firstName'].' '.$user_res['lastName']; ?>" onclick="globalloading();"><img src="images/lock.png" /></a><?php } ?></td>			
          <td align="center" valign="top" class="graylist"><a href="<?php echo $websiteurl;?>preview-project.html?projId=<?php echo encodeStr($res_post['id']);?>" target="_blank"><img src="images/eye_inv.png" alt="View" width="30" height="32" border="0" /></a></td>
        </tr>			
