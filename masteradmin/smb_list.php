...
$sql_ins="update "._BUSINESS_MASTER_TABLE_." set status='$status',emailSent=1  where id = ".$id."";			
...
$mailBodyContent.='<div style="padding:20px 0px; text-align:center; background-color:#FFFFFF;">
    ...
    <img src="'.$websiteurl.'images/logo.png" width="150px;">
    </a>
</div>
  ...
  <div style="margin:auto; width:600px; background-color:#FFFFFF; text-align:left;">
    <div style="padding:30px;">
      ...
      <div style="padding:10px; background-color:#F9F9F9; border:dashed 1px #ccc; border-radius: 2px;">
        ...
        <div style="display: block;width: 100%;margin-top: 15px;">Thank you for creating your Business Page on Konectt. Members will be able to view your page, within the next 24 hours.<br>
          <br>
        You can view the same, by clicking on the "SMB Connect" link on the main menu bar, on the left side. You can edit the particulars at any time and delete the listing as well.<br>
          <br>You may want to create another Business Page, by <a href="'.$websiteurl.'cbp.html" target="_blank">clicking here</a>
</div>
          </div>
        </div>
...
$classlist=1;			
	 $query = "SELECT COUNT(*) as num FROM "._BUSINESS_MASTER_TABLE_."  order by id desc ";			
	...
	$query1 = "SELECT * FROM "._BUSINESS_MASTER_TABLE_."  order by id desc LIMIT $start, $limit";			
	...
	<div class="listingbox" style="position:relative; min-height:300px;">			
	  <table width="100%" border="0" cellpadding="0" cellspacing="0">			
        <tr>
          ...
          <td width="16%" align="left" valign="top" class="listingheaddr">Company Name</td>
          <td width="28%" align="left" valign="top" class="listingheaddr">Short Description </td>
          <td width="14%" align="left" valign="top" class="listingheaddr">Location</td>
          <td width="16%" align="center" valign="top" class="listingheaddr">Address</td>
          <!--<td width="17%" align="left" valign="top" class="listingheaddr">Email id</td>-->			
          <td width="16%" align="center" valign="top" class="listingheaddr"> Date </td>
          <td width="6%" align="center" valign="top" class="listingheaddr"> Posted By</td>
          <td width="7%" align="center" valign="top" class="listingheaddr">Status</td>			
          <td width="8%" align="center" valign="top" class="listingheaddr">Open</td>
        </tr>			
        <?php  while($res_post = mysql_fetch_array($result)) { 
		$postTitle=substr(cleanInput(strip_tags(trim(strtolower(str_replace(' ','-',preg_replace('/[^A-Za-z0-9\-]/', '-', $res_post['companyName'])))))),0,100);
		?>			
		<tr <?php if($classlist==1){ ?>style="background-color:#f7f7f7;" <?php }  else { ?>style="background-color:#FFFFFF;" <?php } ?>>
          ...
          <td align="left" valign="middle" class="graylist"><?php  echo stripslashes($res_post['companyBusinessName']); ?></td>
          <td align="left" valign="middle" class="graylist"><?php  echo stripslashes($res_post['shortDescription']); ?></td>
          <td align="left" valign="middle" class="graylist"><?php  echo stripslashes($res_post['cityName']); ?>, <?php  echo stripslashes($res_post['stateId']); ?>, <?php echo stripslashes($res_post['countryId']); if($res_post['postalCode']!=''){?> - <?php echo $res_post['postalCode']; }?></td>
          <td align="center" valign="middle" class="graylist"><?php  echo stripslashes($res_post['completeAddress']); ?></td>
          <!--<td align="left" valign="middle" class="graylist"><?php  echo stripslashes($user_res['email']); ?></td>-->
          <td align="center" valign="middle" class="graylist"><?php echo date("d-m-Y H:i:s", strtotime($res_post['dateAdded']));?></td>
          <td align="center" valign="middle" class="graylist"><?php $re="select * from "._USERS_MASTER_TABLE_."  where userId='".$res_post['userId']."'";
$re2=mysql_query($re) or die(mysql_error());  $user_res=mysql_fetch_array($re2);
echo $user_res['firstName'].' '.$user_res['lastName'];
?></td>
          <td align="center" valign="middle" class="graylist"><?php if($res_post['status']=='0'){ ?>			
            <a href="<?php echo $backpage; ?>?status=1&id=<?php echo base64_encode($res_post['id']); ?>&unino=<?php echo $_SESSION["cust_session_token"]."_console";?>" onclick="globalloading();"><img src="images/lock.png" /></a>			
            <?php } else { ?><a href="<?php echo $backpage; ?>?status=0&id=<?php echo base64_encode($res_post['id']); ?>&unino=<?php echo $_SESSION["cust_session_token"]."_console";?>&emailSent=<?php echo $res_post["emailSent"];?>&email=<?php  echo stripslashes($user_res['email']); ?>&username=<?php echo $user_res['firstName'].' '.$user_res['lastName']; ?>&pageurl=<?php echo $websiteurl;?><?php echo _SMBURL_TEXT_;?>/<?php echo encodeStr($res_post['id']);?>/<?php echo makeContentUrl($res_post['companyBusinessName']); ?>.html" onclick="globalloading();"><img src="images/unlock.png" width="30" border="0" /></a><?php } ?></td>			
          <td align="center" valign="top" class="graylist"><a href="<?php echo $websiteurl;?><?php echo _SMBURL_TEXT_;?>/<?php echo encodeStr($res_post['id']);?>/<?php echo makeContentUrl($res_post['companyBusinessName']); ?>.html" target="_blank"><img src="images/eye_inv.png" alt="View" width="30" height="32" border="0" /></a></td>
        </tr>			
