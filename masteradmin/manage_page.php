...
$backpage = "manage_page.php"; 
$targetpage = "manage_page.php"; 
$tableName = "post_list";
$addeditpage = "add_page.php"; 	
$limit = 10;
...
$classlist=1;
	 $query = "SELECT COUNT(*) as num FROM $tableName where  type='menu' and id!=7 order by id asc ";
	...
	$query1 = "SELECT * FROM $tableName where   type='menu'  and id!=7  order by id asc LIMIT $start, $limit";
	...
	<div class="listingbox" style="position:relative;">
	  <table width="100%" border="0" cellpadding="0" cellspacing="0">
        <tr>
          ...
          <td width="92%" align="left" valign="top" class="listingheaddr">Page Name</td>
		...
		<tr <?php if($classlist==1){ ?>style="background-color:#f7f7f7;" <?php }  else { ?>style="background-color:#FFFFFF;" <?php } ?>>
         ...
         <td align="center" valign="middle" class="graylist"><?php if($res_post['status']==1){ ?>
            <a href="<?php echo $backpage; ?>?status=2&id=<?php echo $res_post['id']; ?>" onclick="globalloading();"><img src="images/unlock.png" width="30" border="0" /></a>
            <?php } else { ?><a href="<?php echo $backpage; ?>?status=1&id=<?php echo $res_post['id']; ?>" onclick="globalloading();"><img src="images/lock.png" /></a><?php } ?></td>
