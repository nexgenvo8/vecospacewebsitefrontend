...
<?php if ($type == 'scheduleevent') { ?>

<form class="nw-grup" id="scheduleEventForm" style="padding:0px;">

    <!-- MODE FLAG -->
    <input type="hidden" id="eventMode" value="new">

    <label>Google Meeting Link <span class="reqstar">*</span></label>
    <input type="url" id="meetingLink" class="validate"
           placeholder="https://meet.google.com/xxx-xxxx-xxx">

    <label>Meeting Date <span class="reqstar">*</span></label>
    <input type="date" id="meetingDate" class="validate">

    <label>Meeting Time <span class="reqstar">*</span></label>
    <input type="time" id="meetingTime" class="validate">

    <div class="popup-fttr">
        <a onclick="closefuncommonpopupwin();">Cancel</a>
        <button type="button"
                style="background-color:#C02621;color:white"
                onclick="saveScheduleEvent();">
            Save
        </button>
    </div>
...
function formatTime12(time24){
    ...
    return `${h}:${m}:${s.toString().padStart(2,'0')} ${ampm}`;
}

function saveScheduleEvent(){

    let link = $('#meetingLink').val().trim();
    let date = $('#meetingDate').val();
    let time = $('#meetingTime').val();
    let mode = $('#eventMode').val();
    ...
    let prettyTime = formatTime12(time);

    // =========================
    // ✅ EDIT MODE
if(mode === 'edit' && window.parent.editingEvent){

    let box = window.parent.editingEvent.element;
    let msgId = window.parent.editingEvent.stmtId;

    box.attr('data-date', date);
    box.attr('data-time', time);
    box.attr('data-link', link);

    box.find('.event-row:eq(0)').html(`<strong>Date:</strong> ${date}`);
    box.find('.event-row:eq(1)').html(`<strong>Time:</strong> ${prettyTime}`);
    box.find('.event-link').attr('href', link);

    let updatedHtml = box.prop('outerHTML');
    ...
    // =========================
    let eventText = `
<div class="chat-event" data-date="${date}" data-time="${time}" data-link="${link}">

  <div class="event-title">
    📅 Meeting Scheduled
    <span class="edit-event">✏️</span>
  </div>

  <div class="event-row"><strong>Date:</strong> ${date}</div>
  <div class="event-row"><strong>Time:</strong> ${prettyTime}</div>

  <div class="event-row meeting-status"></div>
  <div class="event-row attendance-row"></div>


  <div class="event-row meeting-link-row">
    <strong>Link:</strong>
    ...
    window.parent.$('#sendchat1').val(eventText);
    window.parent.postsmchat();
...
$(document).ready(function(){
    ...
    setTimeout(function(){
        ...
        if(window.parent && window.parent.editingEvent){

            let ev = window.parent.editingEvent;

            console.log("EDIT DATA:", ev); // debug

            $('#eventMode').val('edit');
            $('#meetingLink').val(ev.link);
            $('#meetingDate').val(ev.date);
            $('#meetingTime').val(ev.time);

        }

    }, 150); // wait for iframe sync
...
<?php if($type == 'stmtfeedback'){ ?>

<form style="padding:10px;">
<label>Write Feedback</label>

<textarea id="feedbackText"
style="width:100%;height:120px;border:1px solid #ccc;padding:8px;"></textarea>

<br><br>

<button type="button" onclick="saveFeedback();"
style="background:#28a745;color:white;padding:8px 15px;border:0;">
Save
</button>
</form>
...
function saveFeedback(){
    ...
    let text = $('#feedbackText').val().trim();
    ...
    $.post('save_attendance.php',{
        action:'feedback',
        msgId: window.parent.currentFeedbackId,
        text:text
    },function(){
...
$type = $_REQUEST['type'];
$msgId = isset($_REQUEST['msgId']) ? intval($_REQUEST['msgId']) : 0;

if ($type == 'stmttestimonial') {

	// ==========================
	// 1. FETCH CHAT DATA (ONLY FOR DATE)
	// ==========================
	$chatQry = mysqli_query($conn, "
        SELECT chatText 
	...
	$chatData = mysqli_fetch_assoc($chatQry);
	...
	// 2. EXTRACT ONLY DATE
	// ==========================
	preg_match('/Date\s*:\s*([0-9]{4}-[0-9]{2}-[0-9]{2})/i', $chatText, $dateMatch);
	...
	// ==========================
	// 3. GET STUDENT FROM SESSION
	// ==========================
	$studentId = intval($_SESSION['sessUserId']);

	$studentQry = mysqli_query($conn, "
        SELECT firstName, lastName, registrationNo
        FROM " . _USERS_MASTER_TABLE_ . "
        WHERE userId = '$studentId'
        LIMIT 1
    ");

	$studentData = mysqli_fetch_assoc($studentQry);

	$studentName = trim(($studentData['firstName'] ?? '') . ' ' . ($studentData['lastName'] ?? ''));
	$regNo = $studentData['registrationNo'] ?? '';

	// ==========================
	// 4. FETCH MENTOR NAME (YOUR LOGIC)
	// ==========================
	$sqlLogin = "
        SELECT u.firstName, u.lastName
        FROM " . _STUDENT_REQUEST_MENTOR_FRND_MASTER_TABLE_ . " r
        JOIN " . _USERS_MASTER_TABLE_ . " u ON u.userId = r.mentorId
	...
	$mentorData = mysqli_fetch_assoc($resLogin);

	$mentorName = trim(($mentorData['firstName'] ?? '') . ' ' . ($mentorData['lastName'] ?? ''));
	?>
	...
	<form style="padding:10px;">
		...
		<div class="form-row">
			<div class="form-group">
				...
				<input type="text" id="studentName" value="<?php echo htmlspecialchars($studentName); ?>">
			</div>

			<div class="form-group">
				<label>Registration Number</label>
				<input type="text" id="registrationNumber" value="<?php echo htmlspecialchars($regNo); ?>">
			</div>
		</div>
		...
		<div class="form-row">
			<div class="form-group">
				...
				<input type="date" id="meetingDate" value="<?php echo $meetingDate; ?>">
			</div>

			<div class="form-group">
				<label>Mentor Name</label>
				<input type="text" id="mentorName" value="<?php echo htmlspecialchars($mentorName); ?>">
			</div>
		</div>
		...
		<div class="form-group">
			<label>Learner Remark</label>
			<textarea id="learner" style="height:100px;"></textarea>
		</div>

		<div class="form-group">
			<label>Agenda</label>
			<textarea id="agenda" style="height:100px;"></textarea>
		</div>

		<br>

		<button type="button" onclick="saveTestimonial();"
			style="background:#C02621;color:white;padding:8px 15px;border:0;">
			Save
		</button>
	...
	<script>
		function saveTestimonial() {
			$.post('save_testimonial.php', {
				action: 'testimonial',
				msgId: '<?php echo $msgId; ?>',
				studentName: $('#studentName').val(),
				regNo: $('#registrationNumber').val(),
				meetingDate: $('#meetingDate').val(),
				mentorName: $('#mentorName').val(),
				duration: $('#duration').val(),  // Add this line
				learner: $('#learner').val(),
				agenda: $('#agenda').val()
			}, function (res) {
				if (res.status) {
			...
			}, 'json');  // Add 'json' parameter to automatically parse response
		}
	</script>
...
<?php if($type == 'recording'){ ?>

<form style="padding:10px;" enctype="multipart/form-data" onsubmit="return false;">

<label>Upload Recording</label>

<input type="file" id="recordFile" accept="audio/*,video/*" style="width:100%;margin-top:10px;">
...
function saveRecording(){
    ...
    let fileInput = document.getElementById('recordFile');
    ...
    formData.append('stmtId', window.parent.currentRecordingId);
    ...
    $.ajax({
        ...
        success:function(res){
            console.log(res);
            window.parent.fetchNewChat();
            window.parent.closefuncommonpopupwin();
        },
