...
if (!empty($rowEvents["eventBanner"])) {
...
<div class="timlist" id="387" style="margin-bottom:8px;">
  <div class="hedr">
    ...
    <div class="hdr_right"><a href="<?php echo $fullurl;?>profile/<?php echo encodeStr($userres['userId']);?>/<?php echo $friendnameurl;?>.html"><?php echo stripslashes(trim($userres["firstName"]));?> <?php echo stripslashes(trim($userres["lastName"]));?></a><span class="timelinecontantsubline"></span>
    ...
    <div class="errow-drop"> <span class="errow"><i class="fa fa-ellipsis-h" aria-hidden="true"></i></span>
      <ul class="erow-list">
		<div style="display:none;" id="clipboard<?php echo trim($rowEvents['id']);?>"><?php echo $fullurl."events-detail.html?eventId=".encodeStr($rowEvents['id']);?></div>
		<li>
		<a  style="cursor:pointer;" onclick="copyToClipboard('#clipboard<?php echo trim($rowEvents['id']);?>');"><i class="fa fa-link" aria-hidden="true"></i> Copy link to post</a></li>
      </ul>
    </div>
  ...
  <div class="img"><a href="<?php echo $fullurl; ?>events-detail.html?eventId=<?php echo encodeStr($rowEvents['id']);?>" style="display:block; margin-top:-1px; font-size:16px;"><img src="<?php echo $fullurl; ?>uploads/<?php echo $eventBgphoto;?>">  <span class="evnt-prfl-img"><img src="<?php echo $fullurl; ?>uploads/<?php echo $eventphoto;?>"></span></a></div>
      ...
      <div style="font-size:15px; font-weight:bold; margin-bottom:5px;">
        <ul class="cht_mmbr_list" style="height:auto;    border-radius: 4px; margin-bottom:7px;">
          <li style="border-bottom: 1px #ccc solid;  padding: 16px;  border: 0px; padding-bottom:0px;    background-color: #f2f2f2; margin-bottom:7px;" class="going">
		  
            <div class="frfl-img" style="float:none; border-bottom:px #CCCCCC solid;">
			
			<strong style="width:100%; font-size:16px;"><a href="<?php echo $fullurl; ?>events-detail.html?eventId=<?php echo encodeStr($rowEvents['id']);?>" style="display:block; margin-top:-1px; font-size:16px;"><?php echo stripslashes(str_replace('�','&ndash;',$rowEvents["eventName"]));?></a></strong> <span style="color:#666666; margin-top: 2px; font-weight:normal;"><?php $strstrtdate=strtotime($rowEvents["eventDate"]); echo date("j M",$strstrtdate); ?> - <?php $strenddate=strtotime($rowEvents["eventTillDate"]); echo date("j M Y",$strenddate); ?> <?php echo stripslashes(strip_tags($rowEvents["eventVenue"]));?> <?php echo stripslashes($rowEvents["eventCountryAddress"]);?></span>
	...
	?>
		...
		if($resGroupMembers)
		{
		  ...
		  ?>
				   <li><a href="<?php echo $fullurl;?>profile/<?php echo encodeStr($usergroupmembers['userId']);?>/<?php echo $groupmembernameurl;?>.html"><img title="<?php echo stripslashes(trim($usergroupmembers["firstName"]));?> <?php echo stripslashes(trim($usergroupmembers["lastName"]));?>" alt="<?php echo stripslashes(trim($usergroupmembers["firstName"]));?> <?php echo stripslashes(trim($usergroupmembers["lastName"]));?>" src="<?php echo $fullurl;?>uploads/<?php echo stripslashes(trim($usergroupmemberphoto));?>"></a></li>
                <?php
		}
}
?>
              </ul>
			  <div style="position:absolute; right:0px; top:10px; font-size:12px; color:#999999;"><?php echo $mytotalgroups; if($mytotalgroups>1){ echo ' '.$companNameTitle.' members are going to be there.'; }else{ echo ' '.$companNameTitle.' member is going to be there.';}?> </div>
			</div>
