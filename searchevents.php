...
$pageIndex = 10;
$strWhereEvent = "";
$strSearchEvent = clean($_GET["searchevents"]);
if ($strSearchEvent != '') {
	$strWhereEvent = " and (eventName like '%" . $strSearchEvent . "%' OR eventDetails like '%" . $strSearchEvent . "%' OR eventVenue like '%" . $strSearchEvent . "%' OR eventCountryAddress like '%" . $strSearchEvent . "%' OR eventBrief like '%" . $strSearchEvent . "%' OR eventAgenda like '%" . $strSearchEvent . "%' OR otherDetails like '%" . $strSearchEvent . "%') ";
}
?>
<!DOCTYPE html>
<html>
...
<head>
	<title>Search Events - <?php echo $companNameTitle; ?></title>
...
<body>
				...
				<div
					...
					<div class="evnts">
						...
						<div class="evnt-bnnr" style="background-image: url(images/eventsbanner.jpg);">
							...
							<label>Discover the right events for you</label>
						...
						<div class="evnt-list-cont">
							<ul class="cntr_tab">
								<li><a class="active">Search results for (<?php echo $strSearchEvent; ?>)</a></li>
							</ul>
							<ul class="evnts-list">
								<?php
								$n = 0;

								// Ensure $strWhereEvent is defined
								if (!isset($strWhereEvent)) {
									$strWhereEvent = "";
								}
    ...
    LIMIT 0,50";
							...
							<?php if ($n == 0) { ?>
								<div style="padding:20px; text-align:center;overflow: hidden;">You have no any event.</div>
							<?php } ?>
						</div>
