		...
		<div class="container main">
			...
			<div class="home_container">
							...
							if ($resGroup) {
								...
								<ul class="curently-trendng-list">
									...
									while ($rowgroup = mysqli_fetch_array($resGroup)) {
										...
										if ($row["id"] != '') {
											$sql_postgroup = "SELECT * FROM " . _SHAREANDUPDATES_TABLE_ . " WHERE shareType != 0 AND groupId = " . intval($row["id"]) . " ORDER BY id DESC";
											...
											if ($respostgroup && mysqli_num_rows($respostgroup) > 0) {
												...
												$rowGroupPost = []; // prevent undefined variable warning
												$groupPostTitle = ''; // default empty title
											}
										}
				...
				<div class="list_column">
				  ...
				  if ($row["groupType"] == 0) { ?>groups-detail<?php } else { ?>private-group<?php } ?>.html?groupId=<?php echo encodeStr($rowgroup['id']); ?>"><img src="<?php echo $fullurl; ?>uploads/<?php echo stripslashes(trim($groupThumb)); ?>"></a> <span> <a href="<?php echo $fullurl;
						   if ($row["groupType"] == 0) { ?>groups-detail<?php } else { ?>private-group<?php } ?>.html?groupId=<?php echo encodeStr($rowgroup['id']); ?>" class="nm"><?php echo stripslashes(trim($row["groupName"])); ?></a>
				  ...
				  <?php if ($row["groupDetails"] != '') {
					  echo getStrLength(strip_tags(stripslashes($row["groupDetails"])), 160);
											...
											<div class="currntly-trendingbox">
												...
												<div class="grupbuttons">
													<?php if ($mygroupblockuser == 0) { ?><a
															href="<?php echo $fullurl;
															if ($row["groupType"] == 0) { ?>groups-detail<?php } else { ?>private-group<?php } ?>.html?groupId=<?php echo encodeStr($rowgroup['id']); ?>"
