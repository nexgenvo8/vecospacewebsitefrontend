	...
	if($resLogin)
	{
		...
		else
		{
			...
			if($resUserLoginTime)	
			{
				...
				$mailBodyContent='<div style="padding:20px 0px; text-align:center; background-color:#FFFFFF;"><a href="'.$fullurl.'" style="border:0px;"><img src="'.$fullurl.'images/logo.jpg" width="217" style="border:0px;"></a></div>
...
<div class="banner">
  <div class="container">
   <div class="second-step thankyou-confirm" style="width:500px;" >
   	<?php if($oops==1){?><i class="fa fa-times-circle-o" aria-hidden="true" style="color: #fd6363;"></i><?php }else{?><i class="fa fa-check-circle-o" aria-hidden="true"></i><?php }?>
    <h2 style="text-align:center; color:#1a94c3;"><?php  echo $textVar; ?></h2>
       <form name="frmkonectt" id="frmkonectt" class="personal-data" method="post">   
       	<div style="text-align:center; "> <?php if(trim($errMsg)!='') { ?>
              <div id="emsgdiv" class="<?php echo $className; ?>" style="display:block; margin-top:10px; margin-bottom:10px;"><?php echo $errMsg; ?><br>
                <br>
              </div>
              <?php } ?><a href="<?php echo $fullurl;?>"><button style="background-color:#3ca3ce; float:none; color:#fff; margin-right:10px;" type="button" class="continue-process-btn">Back to login page</button></a></div> 
<br>
